...
st.markdown("""
    <style>
    /* メイン見出し */
    h3 { font-size: 1.4rem !important; font-weight: 600 !important; margin-top: 10px !important; margin-bottom: 5px !important; }
    h4 { font-size: 1.2rem !important; font-weight: 600 !important; margin-top: 15px !important; }
    h5 { font-size: 1.0rem !important; font-weight: 500 !important; color: #aaaaaa !important; margin-top: 10px !important; }
    
    /* キャプション余白 */
    [data-testid="stCaptionContainer"] { margin-top: -15px !important; margin-bottom: 10px !important; }

    /* サイドバー専用スタイル */
    [data-testid="stSidebar"] h2 { font-size: 1.3rem !important; font-weight: 700 !important; }
    [data-testid="stSidebar"] h3 { font-size: 1.1rem !important; font-weight: 600 !important; }
    [data-testid="stSidebar"] .stMarkdown p { font-size: 0.9rem !important; }
...
st.markdown("""
    <div style='margin-bottom: 20px;'>
        <h1 style='font-weight: 400; font-size: 42px; margin: 0; padding: 0;'>BACK TESTER</h1>
        <div style='font-weight: 300; font-size: 18px; margin: 0; padding: 0; color: #aaaaaa;'>DAY TRADING MANAGER｜ver 6.4</div>
    </div>
    """, unsafe_allow_html=True)
...
if st.session_state['view_mode'] == 'individual':
    ...
    if st.button("← 個別銘柄検証に戻る"):
        st.session_state['view_mode'] = 'individual'
        st.rerun()

# --- タブ表示 ---
res_df = st.session_state.get('res_df', pd.DataFrame())
if not res_df.empty or st.session_state['view_mode'] == 'ranking' or 'last_rank_df' in st.session_state:
    tabs = st.tabs(["📊 サマリー", "🏅 勝ちパターン", "📉 ギャップ分析", "🧐 VWAP分析", "🕒 時間分析", "📝 詳細ログ", "🏆 ランキング"])
    
    with tabs[0]: # サマリー
        if not res_df.empty and 'PnL' in res_df.columns:
            count = len(res_df); wins = res_df[res_df['PnL']>0]
            st.markdown(f"""
            <div style='display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 10px;'>
                <div style='background:#262730; padding:15px; border-radius:8px; text-align:center;'>
                    <div style='font-size:12px; color:#aaa;'>回数</div><div style='font-size:24px; font-weight:bold;'>{count}回</div>
                </div>
                <div style='background:#262730; padding:15px; border-radius:8px; text-align:center;'>
                    <div style='font-size:12px; color:#aaa;'>勝率</div><div style='font-size:24px; font-weight:bold;'>{len(wins)/count:.1%}</div>
                </div>
                <div style='background:#262730; padding:15px; border-radius:8px; text-align:center;'>
            ...
            st.divider()
            st.code("\n".join([f"{t}: {len(res_df[res_df['Ticker']==t])} trades" for t in res_df['Ticker'].unique()]))
        else: st.info("個別データはありません。")

    with tabs[5]: # 詳細ログ
        if not res_df.empty:
            log = []
            for _, r in res_df.sort_values('Entry', ascending=False).iterrows():
                log.append(f"{r['Entry'].strftime('%m/%d %H:%M')} | {r['Ticker']} | PnL:{r['PnL']:+.2%} | 買:{int(r['In'])} | 売:{int(r['Out'])} | {r['Reason']}")
            st.code("\n".join(log))
        else: st.info("ログはありません。")

    with tabs[6]: # ランキング
        st.markdown("### 🏆 登録銘柄ランキング")
        if st.session_state.get('trigger_rank_scan'):
            st.session_state['trigger_rank_scan'] = False
            r_list = []; all_t = list(TICKER_NAME_MAP.keys())
            start_date = datetime.now() - timedelta(days=days_back)
            with st.status("🔍 全231銘柄を分析中...", expanded=True) as status:
                pb_r = st.progress(0)
                for i, t in enumerate(all_t):
                    status.update(label=f"Scanning: {t}")
                    df_r = fetch_intraday(t, start_date, datetime.now())
                    if not df_r.empty:
                        cur_p = df_r['Close'].iloc[-1]
                        if params['p_min'] <= cur_p <= params['p_max']:
                            p_map, o_map, a_map = fetch_daily_stats_maps(t, start_date)
                            trades = run_ticker_simulation(t, df_r, p_map, o_map, a_map, params)
                            if trades:
                                tdf = pd.DataFrame(trades)
                                r_list.append({'コード': t, '銘柄名': get_ticker_name(t), '回数': len(tdf), '勝率': (tdf['PnL']>0).mean(), '期待値': tdf['PnL'].mean()})
                    pb_r.progress((i+1)/len(all_t))
            st.session_state['last_rank_df'] = pd.DataFrame(r_list).sort_values('期待値', ascending=False)
            st.rerun()

        if 'last_rank_df' in st.session_state:
            st.dataframe(st.session_state['last_rank_df'].head(20).style.format({'勝率': '{:.1%}', '期待値': '{:+.2%}'}), use_container_width=True, hide_index=True, height=735)
            if st.button("ランキングリセット"): del st.session_state['last_rank_df']; st.rerun()
